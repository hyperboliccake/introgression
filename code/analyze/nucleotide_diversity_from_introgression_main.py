...
def mark_introgressed(seq, regions, fill='N'):
...
for i in range(num_strains):
    ...
    for chrm in gp.chrms:
        ...
        print strain_i, chrm
        strain_i_seqs[chrm] = referize(read_fasta.read_fasta(strain_fn)[1][0].lower(),  ref_ind_to_strain_i_ind)
        strain_i_seqs_nonint[chrm] = copy.deepcopy(strain_i_seqs[chrm])
        if regions_by_chrm_and_strain[chrm].has_key(strain_i):
            strain_i_seqs_nonint[chrm] = mark_introgressed(strain_i_seqs[chrm], regions_by_chrm_and_strain[chrm][strain_i])
    ...
    for j in range(i+1, num_strains):
        ...
        for chrm in gp.chrms:
            ...
            strain_j_seq = referize(read_fasta.read_fasta(strain_fn)[1][0].lower(), ref_ind_to_strain_ind)
            ...
            # excluding introgressed
            if regions_by_chrm_and_strain[chrm].has_key(strain_j):
                strain_j_seq = mark_introgressed(strain_j_seq, regions_by_chrm_and_strain[chrm][strain_j])

            num_chrm_nonint, den_chrm_nonint = count_diffs(strain_i_seqs_nonint[chrm], strain_j_seq)
            num_nonint += num_chrm_nonint
            den_nonint += den_chrm_nonint

            total_fracs_nonint[chrm] += float(num_chrm_nonint)/den_chrm_nonint

            print num_comparisons, chrm, total_fracs[chrm], \
        ...
        print num_comparisons, total_frac, total_frac_nonint, \
            1 - total_frac_nonint/total_frac
        sys.stdout.flush()
...
f = open(gp.analysis_out_dir_absolute + tag + '/polymorphism/' + \
         'nucleotide_diversity.txt', 'w')
f.write('chromosome\tpi\tpi_nonint\n')
