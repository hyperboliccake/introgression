...
ARGS=$(head -n 11 predict_args.txt | tail -n 1)
CHRM=$(head -n $SLURM_ARRAY_TASK_ID chrms.txt | tail -n 1)

python gene_predictions_chrm_main.py $CHRM refs.txt strains.txt $ARGS
