...
###########
## loop through all strains, creating one plot for each
###########

for (strain in strains[1:2]) {

    print(strain)

    ## plot png file
    png(paste('/tigress/AKEY/akey_vol2/aclark4/projects/introgression/',
              'results/analysis/', tag, '/plots/strains/genome_pop_int_', strain, '_',
    ...
    for (ci in 1:length(chrms)) {
        ...
        ## plot population blocks
        a = read.table(paste('/tigress/AKEY/akey_vol2/aclark4/projects/introgression/',
                             'results/analysis/', tag, '/structure/', structure_id,
                             '/population_ranges/', 'population_ranges_', strain,
                             '_chr', chrm, '_run', structure_id, '.txt', sep=''),
                       sep='\t', header=T, stringsAsFactors=F)
        for (r in 1:nrow(a)) {
            region_start = a[r,1]
            region_end = a[r,2]
            if (a[r,3] %in% names(pop_cols)) {
                col = pop_cols[[a[r,3]]]
                rect(region_start,
                     row_bottom,
                     region_end, 
