...
plot(c(region_start, region_end),
     c(-genome_height + 1 - padding * 2 - hmargin,
...
for (i in 1:nrow(variants)) {
    ...
    for (j in strains$index) {
        ...
        if (x == 'p') {
            ...
            segments(ps, positions[j] - strain_height / 2, y1=positions[j] + strain_height/2, col = alpha(my_color_palette[['introgressed']],.5), lend='butt', lwd=.5)
        }
        else if (x == 'c') {
            #points(ps, positions[j], col = alpha(my_color_palette[['nonintrogressed']],.5),pch='|',cex=.3)
            segments(ps, positions[j] - strain_height / 2, y1=positions[j] + strain_height/2, col = alpha(my_color_palette[['nonintrogressed']],0), lend='butt', lwd=.5)
        }
        else if (x == 'n') {
...
rect(region_start, 
     -genome_height + 1 - padding * 2 - hmargin,
     region_end, 
     num_strains * (padding + strain_height) + hmargin,	 
