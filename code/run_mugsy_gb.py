...
for x in c:
    ...
    for chrm in chrms_roman:
        if 'S288c_CBS432_' + x[:-3] + '_chr' + chrm + '.maf' not in a:
            cmd_string += '~/software/mugsy/mugsy --directory ~/introgression/100genomes/chrm/alignments_gb/ --prefix S288c_CBS432_' + x[:-3] + '_chr' + chrm + ' ' + drc + refc[:-3] + '/' + refc[:-3] + '_chr' + chrm + '.fa' + ' ' + drp + 'CBS432/CBS432_chr' + chrm + '.fa' + ' ' + dc  + x[:-3] + '_chr' + chrm + '.fa' + '; '
    print x
